...
Feature: Basic YASL Schema Validation
  ...
  Scenario: Validating a simple YASL schema by specifying model name
    Given a YASL schema "features/yasl/data/person.yasl" is provided
    And a YAML document "features/yasl/data/person.yaml" is provided
...
Scenario: Validating a simple YASL schema without specifying model name
  ...
  Scenario: Validating a directory of YASL files
    Given a YASL schema "features/yasl/data/dir_test" is provided
    And a YAML document "features/yasl/data/dir_test" is provided
    When I run the YASL CLI with provided arguments
    Then the validation should pass

  Scenario: Validating a bad directory of YASL files
    Given a YASL schema "features/yasl/data/bad_dir_test" is provided
    And a YAML document "features/yasl/data/bad_dir_test" is provided
    When I run the YASL CLI with provided arguments
