...
def given_schema_step_impl(context, schema_file):
...
def given_document_step_impl(context, document_file):
...
def given_model_name_step_impl(context, model_name):
    context.model_name = model_name

@when('I run the YASL CLI with provided arguments')
def when_run_yasl_cli_step_impl(context):
...
def then_validation_passed_step_impl(context):
    assert context.validation_successful is True, "Validation failed unexpectedly!"
    print("Validation passed!")

@then('the validation should fail')
def then_validation_failed_step_impl(context):
